...
void luat_shell_poweron(int _drv) {
    ...
    esp_err_t  ret=uart_driver_install(1, 2048, 2048, LUAT_UART_QUEUE_SIZE, &(uart_port[1].xQueue), 0);
    if (ret != ESP_OK) {
        printf("UART1 driver install failed! Error: %d\n", ret);
        return;
    }
    uart_set_pin(1, 33/*TX*/ /* UART_PIN_NO_CHANGE*/,32/*RX*//* UART_PIN_NO_CHANGE*/, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
    uart_set_baudrate(1, 921600);
    // uart_param_config(0, &uart_config);
    uart_pattern_queue_reset(1, LUAT_UART_QUEUE_SIZE);
    if (uart_port[1].xQueue != NULL) {
        xTaskCreate(uart1_irq_task, "uart1_irq_task", 4096, NULL, 12, &uart_port[1].xHandle);
    } else {
        printf("Error: UART1 queue not created!\n");
    }
    xTaskCreate(uart1_irq_task, "uart1_irq_task", 4096, NULL, 12, &uart_port[1].xHandle);
    printf("UART1 driver installed.\n");
}
#endif
